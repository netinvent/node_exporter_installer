...
verify_init_system() {
  ...
  # Generic initv (also detects upstart in ubuntu 14.04)
  if [ -x /sbin/init ]; then
    INIT_SYSTEM=initv
    return
  fi
  ...
  # Not supported
  fatal 'No supported init system found (OpenRC, upstart, initV or systemd)'
}
...
create_service_file() {
  case $INIT_SYSTEM in
    openrc|upstart|initv) create_openrc_service_file ;;
...
service_enable_and_start() {
  ...
  case $INIT_SYSTEM in
    openrc|upstart|initv) openrc_enable ;;
  ...
  case $INIT_SYSTEM in
    openrc|upstart|initv) openrc_start ;;
